<script>
	...
	export let translations = {
		title: language.name ?? '',
		subtitle: language.autonym ?? '',
		breadcrumbs: [],
	};

	let rows

	const bible_row = (row) =>  `<tr>
...
const bibles = language.bibles.map(bible => {
	return {
		tt: bible.title,
		tv: bible.title_vernacular,
		dt: bible.date,
		ci: bible.country_id,
		cn: bible.country?.name ?? ''
	}
})
...
<div class="flex-grow">
	...
	<TabPanels>
		{#if bibles.length != 0}
			<TabPanel>
				<Datatable inputData="{bibles}" tableType="language_bibles" />
			</TabPanel>
		{/if}
		{#if language.films.length != 0}
			<TabPanel>
				<Datatable inputData="{language.films}" tableType="films" />
			</TabPanel>
		{/if}
		{#if language.resources.length != 0}
			<TabPanel>
				<Datatable inputData="{language.resources}" tableType="resources" />
			</TabPanel>
		{/if}
	</TabPanels>
